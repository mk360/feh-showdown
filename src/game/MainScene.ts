...
function pixelsToGrid(x: number, y: number) {
    return {
        ...
        y: Math.round((y - verticalOffset) / squareSize)
    };
}
...
function createHeroQuoter(scene: MainScene) {
    ...
    return (hero: Hero) => {
        ...
        const { value: name } = internalHero.components.find((c) => c.type === "Name")!;
        const n = scene.rng.integerInRange(1, 3);
        if (previousQuote) scene.sound.stopByKey(previousQuote);
        const soundFile = name + " quotes";
        scene.sound.playAudioSprite(name + " quotes", n.toString(), { volume: 0.2 });
